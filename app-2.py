...
def api_results():
    if pipeline_state["results"]: return jsonify(pipeline_state["results"])
    p = ROOT/"data"/"tables"/"synthetic_tables.json"
    if p.exists(): return jsonify({"tables":json.loads(p.read_text("utf-8")),"preview":True})
...
function renderAll(data){
  ...
  tables.forEach(t=>{
    (t.columns||[]).forEach(col=>{
      let rs=col.risk_score;
      if(rs===undefined){const q=col.description_quality||'';rs=col.risk_level==='HIGH_RISK'?70:q==='missing'?80:q==='wrong'?75:q==='english'?60:q==='vague'?45:q==='incomplete'?30:15;}
      const band=rs>=60?'high':rs>=30?'mid':'low';
      let cl=col.clarity_score;if(cl===undefined)cl=hCl(col);
      const issues=[...(col.issues||[])];
      if(col.validation_issue)issues.unshift('VALUE MISMATCH: '+col.validation_issue);
      if(col.distinct_count&&col.distinct_count<100&&!col.has_lookup)issues.push(`LOOKUP GAP: ${col.distinct_count} distinct, no LKP table`);
      COLS.push({table:t.table_name,schema:t.schema||'',col:col.column_name,desc:col.description||'',origDesc:col.original_description||null,quality:col.description_quality||'',rs,band,cl,issues,feedback:col.feedback||'',hasFrd:t.has_functional_doc||false,hasToa:t.has_toa_doc||false,hasDdl:t.has_ddl||false,deductions:col.clarity_deductions||[],bonuses:col.clarity_bonuses||[],breakdown:col.clarity_breakdown||null});
    });
  });
  const n=COLS.length,hi=COLS.filter(c=>c.band==='high').length,mi=COLS.filter(c=>c.band==='mid').length,lo=COLS.filter(c=>c.band==='low').length;
  const avgC=n?Math.round(COLS.reduce((s,c)=>s+c.cl,0)/n):0;
  ...
  const rs2=(data.risk_report||{}).summary||{};
  if(rs2.yuksek_risk_count!==undefined){set('sHi',rs2.yuksek_risk_count);set('sMid',rs2.orta_risk_count);set('sLo',rs2.dusuk_risk_count);}
...
function showPopup(table,colName){
  ...
  document.getElementById('ppBefore').innerHTML=col.origDesc?`<span class="cb-text">${col.origDesc}</span>`:`<span class="cb-null">(no description)</span>`;
  document.getElementById('ppAfter').innerHTML=col.desc?`<span class="cb-text">${col.desc}</span>`:`<span class="cb-null">(not generated)</span>`;
